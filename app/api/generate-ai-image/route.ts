...
CRITICAL TRANSPARENCY REQUIREMENT: The background MUST be completely transparent. You MUST generate this as a PNG image with RGBA color format (4 channels: Red, Green, Blue, and Alpha). The alpha channel must be present and set to 0 (fully transparent) for all background pixels. Do NOT use any solid color background (no white, no black, no gray, no color). The background must be truly transparent with alpha channel support. Only the main design elements should be visible with opaque pixels.`;
    ...
    const processImageForTransparency = async (
      dataUrl: string,
    ): Promise<string> => {
      try {
    ...
    const detectBackgroundColor = (
      ...
      for (
        let x = 0;
        x < width;
        x += Math.max(1, Math.floor(width / sampleSize))
      ) {
        // Top edge
      ...
      for (
        let y = 0;
        y < height;
        y += Math.max(1, Math.floor(height / sampleSize))
      ) {
        // Left edge
    ...
    const removeBackground = async (dataUrl: string): Promise<string> => {
      try {
        ...
        console.log(
          `Detected background color: RGB(${bgColor.r}, ${bgColor.g}, ${bgColor.b})`,
        );
        ...
        for (let y = 0; y < height; y++) {
          for (let x = 0; x < width; x++) {
            ...
            const saturation =
              maxColor === 0 ? 0 : (maxColor - minColor) / maxColor;
            ...
            const isLowSaturation =
              saturation < lowSaturationThreshold && brightness > 200;
            const isNearWhite =
              brightness > 240 && Math.abs(r - g) < 10 && Math.abs(g - b) < 10;
            const isNearBgColor = colorDist < 50 && brightness > 180;

            // Mark as background if it meets multiple criteria
            if (
              (isVeryBright && isLowSaturation) ||
        ...
        for (let x = 0; x < width; x++) {
          ...
          if (isBackground[(height - 1) * width + x])
    ...
    const generateSingleImage = async (
      ...
      const apiUrl = useFallback
        ...
        : GEMINI_API_URL_FALLBACK; // this is on purpose to use the fallback model
          ...
          if (
            ...
            if (imagePart) {
              ...
              if (inlineData && inlineData.data) {
                ...
                console.log(
                  `Processing image ${imageIndex + 1} for transparency...`,
                );
                try {
                  // First, ensure alpha channel exists
                  const withAlpha =
                    await processImageForTransparency(originalDataUrl);
                  // Then, try to remove white/light backgrounds
                  const withTransparency = await removeBackground(withAlpha);
                  console.log(
                    `✓ Image ${imageIndex + 1} processed successfully with transparency`,
                  );
                  return withTransparency;
                } catch (processError) {
                  console.error(
                    `Error processing image ${imageIndex + 1} for transparency:`,
                    processError,
                  );
