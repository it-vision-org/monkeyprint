  ...
  return (
    <div className={styles.colorPickerComponent}>
      ...
      <div className={styles.colorPreview}>
        ...
        />
        <span className={styles.colorPreviewValue}>
...
const DesignEditor = memo(function DesignEditor({
  productType,
  productColor,
  initialDesign,
  onDesignChange,
  ...
  const designsRef = useRef<Record<Side, string | null>>({
    front: null,
    back: null,
  });
  ...
  const historyRef = useRef<Record<Side, HistoryState[]>>({
    front: [],
    back: [],
  });
  ...
  const [originalImages, setOriginalImages] = useState<Record<string, string>>(
    {},
  ); // Store original image data URLs by object id
  const [bgRemovalTechnique, setBgRemovalTechnique] = useState<
    ...
    fetch("/fonts/fonts.json")
      ...
      .then((data: { fonts: FontConfig[] }) => {
        const fontList = data.fonts || [
          { name: "Arial", file: null, system: true },
        ];
        setFonts(fontList);
        fontList.forEach((f) => {
          if (f.file) {
  ...
  const calculateSize = useCallback(() => {
    ...
    if (mobile) {
      ...
      const availableHeight =
        window.innerHeight - headerHeight - toolbarHeight - 32;

      const w = Math.min(
        availableWidth,
        isNativeFullscreen ? Math.min(520, availableWidth) : 380,
      );
      // Calculate height based on available space, but maintain reasonable aspect ratio
      const h = isNativeFullscreen
        ? Math.min(
            Math.max(w * 1.2, availableHeight * 0.85),
            availableHeight,
  ...
  useEffect(() => {
    if (mainCanvas.current && productImgs.current.front) {
      setBackground(
        ...
        currentSideRef.current === "front" ? printAreaFront : printAreaBack,
      );
    }
    if (previewCanvas.current && productImgs.current.front) {
      setBackground(
        ...
        currentSideRef.current === "front" ? printAreaBack : printAreaFront,
      );
    }
  ...
  const setBackground = useCallback(
    async (
      ...
      const imgElement =
        side === "front" ? productImgs.current.front : productImgs.current.back;
      if (!imgElement) {
        canvas.renderAll();
        return;
      }
  ...
  const loadProductImages = useCallback(() => {
    ...
    Promise.all([loadImg(frontImageSrc), loadImg(backImageSrc)]).then(
      ([front, back]) => {
  ...
  const pushToHistory = useCallback((preSerializedState?: string) => {
    if (isUndoingRef.current || !mainCanvas.current || isInitializing.current)
      return;

    const side = currentSideRef.current;
    const serialized = preSerializedState || serialize(mainCanvas.current);

    historyRef.current[side] = historyRef.current[side].slice(
      0,
      historyIndexRef.current[side] + 1,
    );
  ...
  const loadDesign = useCallback(
    async (canvas: fabric.StaticCanvas, design: string | null, side: Side) => {
      ...
      try {
        ...
        const sx = canvas.getWidth() / w,
          sy = canvas.getHeight() / h;
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (
        mainCanvas.current &&
        (mainCanvas.current.getActiveObject() as any)?.isEditing
      )
        return;
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (mainCanvas.current && !isInitializing.current) {
        ...
        designsRef.current[currentSideRef.current] = serialize(
          mainCanvas.current,
        );
        const designData = JSON.stringify(designsRef.current);
  ...
  useEffect(() => {
    ...
    loadDesign(
      previewCanvas.current,
      designsRef.current[oppositeSide],
      oppositeSide,
    );
  ...
  const addImage = (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      fabric.FabricImage.fromURL(url).then((img: fabric.FabricImage) => {
        ...
        const scale = Math.min(
          (printW * 0.8) / (img.width || 1),
          (printH * 0.8) / (img.height || 1),
          1,
        );
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (
        fontDropdownOpen &&
        !(e.target as HTMLElement).closest(`.${styles.fontDropdown}`)
      ) {
        setFontDropdownOpen(false);
      }
    };
  ...
  return (
    <div
      ref={containerRef}
      className={`${styles.editor} ${isFullscreen ? styles.fullscreen : ""} ${isMobile ? styles.mobile : ""}`}
      tabIndex={0}
    >
      {isLoading && (
      ...
      {(showPanel || activeTool === "draw") && (
        <div className={styles.panel}>
          ...
          <div className={styles.panelBody}>
              ...
              <>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    updateTextProp("fontSize", +e.target.value);
                  }}
                />
            ...
            {(selected?.type === "image" ||
              (selected && selected instanceof fabric.FabricImage)) && (
              <div className={styles.imageProps}>
                ...
                {selected &&
                  (selected as any).id &&
                  hasRemovedBg[(selected as any).id] && (
                    <div
                      style={{
                        display: "flex",
                        flexDirection: "column",
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              fabric.FabricImage.fromURL(originalDataUrl)
                                .then((originalImg: fabric.FabricImage) => {
                                  if (!mainCanvas.current) return;
                                  ...
                                  mainCanvas.current.setActiveObject(
                                    originalImg,
                                  );
                                  mainCanvas.current.renderAll();
                              ...
                              console.error(
                                "Error undoing background removal:",
                                error,
                              );
                              alert(
                                error.message ||
