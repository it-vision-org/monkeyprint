...
export default function OrderActions({ orderId, status, deletionRequested }: { orderId: string, status: string, deletionRequested?: boolean }) {
    ...
    return (
        ...
        >
            <button className="commande-delete-btn" onClick={handleDelete} title="Demander la suppression">
            ...
            {status === 'PENDING' && (
                <button className={`commande-check-btn`} onClick={handleConfirm} title="Confirmer">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
